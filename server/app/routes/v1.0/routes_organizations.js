    ...
    app.post('/organizations/:orgId/businessgroups/:bgId/projects/:projectId/environments/:envId/addInstance', function (req, res) {
        ...
        usersDao.haspermission(user.cn, category, permissionto, null, req.session.user.permissionset, function (err, data) {
            ...
            masterUtil.getParticularProject(req.params.projectId, function (err, project) {
                ...
                var queryObj = {
                    'masterDetails.orgId': req.params.orgId,
                    '$or': [{
                        'resourceDetails.publicIp': req.body.fqdn
                    }, {
                        'resourceDetails.privateIp': req.body.fqdn
                    }, {
                        'configDetails.nodeName': req.body.fqdn
                    }],
                    isDeleted: false
                }
                resourceModel.getResources(queryObj, function (err, instances) {
                    if (err) {
                        logger.error("error occured while fetching instances by IP", err);
                        return res.status(500).send(errorResponses.db.error);
                    }
                    if (instances.length && instances[0].category === 'managed') {
                        return res.status(400).send({message: "An Instance with the same IP already exists."});
                    }
                    if (req.body.credentials && req.body.credentials.username && (req.body.credentials.password || req.body.credentials.pemFileData)) {
                        configmgmtDao.getEnvNameFromEnvId(req.params.envId, function (err, envName) {
                            if (err) {
                                return res.send(500);
                            }
                            credentialCryptography.encryptCredential(req.body.credentials, function (err, encryptedCredentials) {
                                if (err) {
                                ...
                                monitorsModel.getById(req.body.monitorId, function (err, monitor) {
                                    ...
                                    var resourceObj = {
                                        ...
                                        resourceDetails: {
                                            ...
                                            publicIp: req.body.fqdn,
                                            state: 'running',
                                            bootStrapState: 'bootStrapping',
                                            credentials: encryptedCredentials
                                        },
                                        configDetails: {
                                            id: req.body.configManagmentId,
